...
- 🎙️ **实时语音识别**：自动将面试官的语音转换为文字，转录结果实时显示。
...
- **STT（语音转文字）**：使用本地运行的 [sherpa-onnx](https://github.com/k2-fsa/sherpa-onnx)，**完全免费**，无需联网，无数据上传。
- **LLM（AI 答题）**：以 [Groq](https://console.groq.com/) 为例，`llama-3.3-70b-versatile` 模型目前对免费层用户**免费**，付费层价格约为 $0.59 / 百万输入 token。一场 1 小时的面试通常花费**不到 $0.10**。
- **综合费用**：一个月密集面试（20 场）的 LLM 成本通常在 **$1–$2** 以内，远低于竞品月订阅费用的零头。
...
| `Integrated` | 浏览器麦克风 → 后端 → sherpa STT → 手动发送 → Groq 流式输出 |
