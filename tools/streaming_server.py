...
class StreamingServer(object):
    def __init__(
        ...
        self.doc_root = str(doc_root or "").strip()
        self.http_server = None
        if HttpServer is not None and self.doc_root and Path(self.doc_root).is_dir():
            self.http_server = HttpServer(self.doc_root)
    ...
    async def process_request(
        ...
        if "sec-websocket-key" not in (
            ...
            if path in ("/", "/index.html") and self.http_server is None:
                response = r"""
<!doctype html><html><head>
<title>Sherpa streaming server</title><body>
<h2>Sherpa streaming server is running.</h2>
<p>The static demo pages are not bundled in this repository.</p>
</body></head></html>
"""
                found = True
                mime_type = "text/html"
            ...
            elif path in ("/upload.html", "/offline_record.html"):
                response = r"""
<!doctype html><html><head>
...
"""
                found = True
                mime_type = "text/html"
            elif self.http_server is not None:
                found, response, mime_type = self.http_server.process_request(path)
            else:
                response = r"""
<!doctype html><html><head>
<title>Sherpa streaming server</title><body>
<h2>404 ERROR! Static demo files are not available in this repository.</h2>
</body></head></html>
"""
                found = False
                mime_type = "text/html"
...
def main():
    ...
    if doc_root and not Path(doc_root).is_dir():
        logging.warning(
            "Directory %s does not exist. Static demo pages will be disabled.",
            doc_root,
        )
        doc_root = ""
    ...
    if HttpServer is None:
        logging.warning(
            "http_server.py is not available. Static demo pages will be disabled."
        )
